...
router.put("/:id", async function (req, res) {
  try {
    let body = req.body;
    let user = await customerModel.findByIdAndUpdate(req.params.id, body, {
      new: true,
    });
    var customers = await customerModel.find().sort({ createdAt: -1 });

    res.json({ message: STRINGS.TEXTS.customerUpdated, customers });
  } catch (error) {
